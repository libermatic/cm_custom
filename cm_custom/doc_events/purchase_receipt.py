...
def set_or_create_batch(doc, method):
    def set_existing_batch(item):
        ...
        if item.cm_mfg_date:
            item.batch_no = frappe.db.exists(
                "Batch",
                {"item": item.item_code, "manufacturing_date": item.cm_mfg_date},
            )
        elif item.cm_exp_date:
            item.batch_no = frappe.db.exists(
                "Batch", {"item": item.item_code, "expiry_date": item.cm_exp_date},
            )
    ...
    get_batch_in_previous_items = compose(
        ...
        lambda x: filter(
            ...
            and (
                item.cm_mfg_date == x.cm_mfg_date or item.cm_exp_date == x.cm_exp_date
            ),
            doc.items,
        ),
    )
            ...
            batch = frappe.get_doc(
                {
                    ...
                    "manufacturing_date": _get_mfg_date(
                        item.cm_mfg_date, item.cm_exp_date, shelf_life_in_days
                    ),
                    "expiry_date": item.cm_exp_date,
