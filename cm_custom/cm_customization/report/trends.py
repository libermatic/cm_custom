...
def get_columns(filters, trans):
    ...
    conditions = {
        ...
        "addl_tables_relational_cond": based_on_details.get(
...
def validate_filters(filters):
    ...
    if not frappe.db.exists("Fiscal Year", filters.get("fiscal_year")):
        frappe.throw(
            _("Fiscal Year {0} Does Not Exist").format(filters.get("fiscal_year"))
        )
            ...
            for dimension in accounting_dimensions:
                if not dimension.disabled and filters.get(dimension.fieldname):
                    if frappe.get_cached_value(
                        ...
                        cond += " and t1.{0} in %({0})s".format(dimension.fieldname)
                    else:
                        cond += " and t1.{0} = %({0})s".format(dimension.fieldname)
    ...
    if filters.get("group_by"):
        ...
        data1 = frappe.db.sql(
            ...
            values=filters.update(
                {"year_start_date": year_start_date, "year_end_date": year_end_date}
            ),
            as_list=1,
        )
...
def get_period_date_ranges(period, fiscal_year=None, year_start_date=None):
    ...
    for _i in range(1, 13, increment):
        period_end_date = getdate(year_start_date) + relativedelta(
            months=increment, days=-1
        )
